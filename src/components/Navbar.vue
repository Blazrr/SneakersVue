<template>
  <div class="p-6 flex items-center justify-between md:p-8 w-full z-10">
    ...
    <div class="flex items-center space-x-4 md:space-x-8">
      <div class="relative">
        ...
        <span
          v-if="store.count > 0"
          class="absolute -top-2 -right-2 bg-orange-300 rounded-lg px-1"
          >{{ store.count }}</span
        >
      </div>
      <img src="../images/image-avatar.png" alt="" class="w-8 md:w-12 cursor-pointer" />
    </div>
  </div>
  ...
  <div
    :class="[isActive ? 'left-0' : 'left-[-300px]']"
    class="absolute top-0 z-30 transition-all"
  >
